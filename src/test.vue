...
export default {
  ...
  mounted() {
    this.content = '<p dir="auto"><span data-katex="true">$\\overline {AD}$</span>نیم ساز <span data-katex="true">$\\mathop{A}\\limits^{\\wedge}$</span> است.</p>'
        + '<span data-katex="true">$\\begin{array}{l}m({\\mathop{A}\\limits_1^{\\wedge}})=m({\\mathop{A}\\limits_2^{\\wedge}}) \\\\ m({\\mathop{D}\\limits_2^{\\wedge}})=m({\\mathop{A}\\limits_1^{\\wedge}})+m(\\mathop{B}\\limits^{\\wedge}) \\\\ m({\\mathop{D}\\limits_1^{\\wedge}})=m({\\mathop{A}\\limits_2^{\\wedge}})+m(\\mathop{C}\\limits^{\\wedge})\\end{array}$</span>'
        + '<p dir="auto">در<span data-katex="true">$m({\\mathop A\\limits^ \\wedge  _1}) &amp;lt; m(A\\mathop D\\limits^ \\wedge  B),B\\mathop A\\limits^\\Delta  D$</span> ، زیرا <span data-katex="true">$m(A\\mathop D\\limits^ \\wedge  B) = m({\\mathop A\\limits^ \\wedge  _2}) + C$</span>. پس: <span data-katex="true">$BD &amp;lt; AB$</span></p><p dir="auto"></p><p dir="auto"></p>'
  },
  methods: {
